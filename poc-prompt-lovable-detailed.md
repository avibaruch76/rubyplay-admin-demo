# Lovable Continuation Prompt — Exhaustive Detail

Paste this into the Lovable chat for project `8c00734f-735f-4f6d-a82e-985dfc6a5311`.

**Use the prompt block below as a single paste.** It's intentionally exhaustive — every screen, every tab, every field, every modal is listed. Lovable can read all of it.
...
- Continuation overview: https://raw.githubusercontent.com/avibaruch76/rubyplay-admin-demo/main/poc-prompt-lovable-continuation.md

STACK YOU'RE USING:
- TanStack Start + Lovable Cloud (Supabase under the hood)
- Tailwind + shadcn/ui components
- Auto-generated REST API at /rest/v1/* via Supabase
...
All images stored in Supabase Storage under /games/[game-id]/[slot-name].png. Make them retrievable via the API as game.hero_image_url, game.banner_url, etc.
...
THIS IS THE MAGIC VIEW. Lovable will likely miss it. Build it carefully.
...
Every games, studios, news_posts, static_pages table gets these columns:
- draft_data (JSONB) — the staging state (what's being edited)
- published_data (JSONB) — the live state (what visitors see on rubyplay.com); null if never published
...
Plus a separate pending_changes table that tracks all items awaiting publish:
...
3. Invalidate CDN cache for all affected URLs
...
- A background job (Supabase cron / pg_cron) fires the publish at the scheduled time
...
PERMISSION GATING (Supabase Row-Level Security):
...
The pending_changes table's INSERT policy enforces this:
INSERT INTO pending_changes auto-sets review_state based on auth.jwt's role claim.

VERSION HISTORY + ROLLBACK:

Each record has a versions table:
- record_id, version_number, snapshot (full JSONB), published_at, published_by
...
- user_id, action, target_record_type, target_record_id, details (JSONB), created_at
- Surfaced in Users & roles → Activity audit tab with filters

NOTIFICATIONS:

When a Studio Lead's submission is approved or rejected, send:
- Email to submitter (via Resend or Supabase Auth)
...
CRITICAL: Permissions must ACTUALLY apply via Supabase Row-Level Security:
...
✅ Curl /rest/v1/games?studio_id=koala&status=live → returns 2 games as JSON
✅ Curl /rest/v1/games?series_id=j-mania → returns 2 games (Bufón + J Mania Goal Nations)
✅ Login as Maya (Studio Lead, Koala) → Games view shows ONLY Koala's games (RLS enforced)
...
9. Wire Supabase Row-Level Security to actually enforce roles
...
Lovable will likely chunk this into 3–5 generations. Some tips:

- **If Lovable says "this is too much"** → tell it to start with steps 1 + 2 (game editor + per-row loading), then ask for the rest in follow-up messages
- **If a phase comes out generic** → say "look at https://rubyplay-admin-demo.onrender.com → [specific view] → the [X] should look like [Y]"
- **If permissions don't enforce** → "Add Supabase Row-Level Security policies: studio_lead users can only SELECT/UPDATE games where studio_id matches their assigned_studio_id from the user profile table"
